...
When contract text enters the system, `dlp/tokenizer.py` calls the Cloud DLP inspect API to detect PII across seven infoTypes: PERSON_NAME, EMAIL_ADDRESS, PHONE_NUMBER, US_SOCIAL_SECURITY_NUMBER, US_INDIVIDUAL_TAXPAYER_IDENTIFICATION_NUMBER, CREDIT_CARD_NUMBER, and STREET_ADDRESS. DLP returns findings with byte offsets and quoted values. The tokenizer replaces each finding in reverse offset order (to preserve positions) with a reversible surrogate token like `[PERSON_NAME_1]` or `[EMAIL_ADDRESS_2]`. The token-to-original mapping is stored in a `TokenizationContext` object. After the LLM returns its analysis, `detokenize()` replaces all tokens in the output with the original values. The LLM sees only tokenized text at every point in the pipeline.

The minimum likelihood threshold is set to `LIKELY`, which reduces false positives on legal text without sacrificing coverage of genuine PII. If the Cloud DLP API is unavailable (network failure, quota exceeded, or missing credentials), the tokenizer falls back to a local regex layer covering EMAIL_ADDRESS, PHONE_NUMBER, and US_SOCIAL_SECURITY_NUMBER and logs a warning. This ensures the pipeline degrades gracefully rather than failing hard.
